...
@section('content')
    ...
    <div class="hero-card mb-0">
        <div class="d-flex align-items-center mb-3">
            ...
            <div>
                <h4 class="text-white mb-1">Edit Wallet</h4>
                <p class="text-muted mb-0">Perbarui informasi wallet Anda</p>
            </div>
        </div>
    </div>
    ...
    <div class="content-section">
        <form action="{{ route('member.dompet.update', $wallet) }}" method="POST">
            @csrf
            @method('PUT')

            <!-- Wallet Type Selection -->
            <div class="card-dark p-4 mb-3">
                <h6 class="text-white mb-3">Tipe Wallet</h6>

                <div class="row g-2">
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="wallet_type" id="type_bank" value="bank"
                            {{ old('wallet_type', $wallet->wallet_type) == 'bank' ? 'checked' : '' }}>
                    ...
                    <div class="col-6">
                        <input type="radio" class="btn-check" name="wallet_type" id="type_ewallet" value="ewallet"
                            {{ old('wallet_type', $wallet->wallet_type) == 'ewallet' ? 'checked' : '' }}>
            ...
            <!-- Bank Form -->
            <div class="card-dark p-4 mb-3" id="bank-form"
                style="{{ $wallet->wallet_type == 'bank' ? 'display: block;' : 'display: none;' }}">
                ...
                <div class="mb-3">
                    ...
                    <select name="bank_name" class="form-select bg-dark text-white border-secondary">
                        ...
                        @foreach (\App\Models\Wallet::BANK_PROVIDERS as $key => $value)
                            <option value="{{ $key }}"
                                {{ old('bank_name', $wallet->bank_name) == $key ? 'selected' : '' }}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="bank_account" class="form-control bg-dark text-white border-secondary"
                        placeholder="Masukkan nomor rekening" value="{{ old('bank_account', $wallet->bank_account) }}">
                    @error('bank_account')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="account_name" class="form-control bg-dark text-white border-secondary"
                        placeholder="Nama sesuai rekening" value="{{ old('account_name', $wallet->account_name) }}">
                    @error('account_name')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <!-- E-wallet Form -->
            <div class="card-dark p-4 mb-3" id="ewallet-form"
                style="{{ $wallet->wallet_type == 'ewallet' ? 'display: block;' : 'display: none;' }}">
                ...
                <div class="mb-3">
                    ...
                    <select name="ewallet_provider" class="form-select bg-dark text-white border-secondary">
                        ...
                        @foreach (\App\Models\Wallet::EWALLET_PROVIDERS as $key => $value)
                            <option value="{{ $key }}"
                                {{ old('ewallet_provider', $wallet->ewallet_provider) == $key ? 'selected' : '' }}>
                                {{ $value }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="ewallet_number" class="form-control bg-dark text-white border-secondary"
                        placeholder="08xxxxxxxxxx" value="{{ old('ewallet_number', $wallet->ewallet_number) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="ewallet_name" class="form-control bg-dark text-white border-secondary"
                        placeholder="Nama sesuai akun e-wallet" value="{{ old('ewallet_name', $wallet->ewallet_name) }}">
                    @error('ewallet_name')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="card-dark p-4 mb-3">
                ...
                <div class="mt-3">
                    ...
                    <textarea name="notes" class="form-control bg-dark text-white border-secondary" rows="2"
                        placeholder="Catatan tambahan...">{{ old('notes', $wallet->notes) }}</textarea>
                    @error('notes')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize on load based on existing data
            toggleForms();
        });
    </script>
@endsection
